  ...
  <div v-else>
    ...
    <div class="mx-auto max-w-7xl px-4 py-6">
      ...
      <div class="grid gap-6 lg:grid-cols-2 mb-8">
        ...
        <div class="space-y-6">
          ...
          <div>
            ...
            <p class="mt-2 text-xs sm:text-sm text-slate-500">{{ $t('productDetail.articleNumber') }}: {{ product.articleNumber }}</p>
          </div>
            ...
            <div v-for="(variant, variantName) in product.variants" :key="variantName" class="space-y-2">
              ...
              <div v-if="variant.type === 'color'" class="flex flex-wrap gap-2">
                <button
                  ...
                  :title="option.name + (option.inStock ? '' : ' - ' + $t('productDetail.unavailable'))"
                >
                  <img
                    v-if="option.image"
                    :src="option.image"
          ...
          <div v-if="product.available" class="rounded-lg border border-slate-200 bg-amber-50 p-3">
            <p class="text-sm font-medium text-amber-900">
              {{ $t('productDetail.urgencyMessage') }}
            </p>
          </div>
            ...
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1">
                <p class="text-sm font-semibold text-slate-900 mb-1">
                  {{ $t('productDetail.alsoOnAmazon') }}
                </p>
                <p class="text-xs text-slate-600 mb-3">
                  {{ $t('productDetail.notInStockFindOnAmazon') }}
                </p>
                <a
                  :href="amazonLink"
                  target="_blank"
                ...
                >
                  <span>{{ $t('productDetail.viewOnAmazon') }}</span>
          ...
          <div class="flex items-center gap-4">
            <label class="text-sm font-medium text-slate-700">{{ $t('productDetail.quantity') }}:</label>
          ...
          <div class="space-y-3">
            ...
            >
              {{ adding ? $t('productDetail.adding') : $t('productDetail.buyNow') }}
            </button>
          ...
          <div class="rounded-lg border border-slate-200 bg-white p-4 space-y-3">
            <div class="flex items-center gap-2 text-brand">
              <Check class="h-5 w-5" />
              <p class="font-semibold">{{ $t('productDetail.shippingReturn') }}</p>
            </div>
            <div class="space-y-2 text-sm text-slate-700">
              <div>
                <span class="font-medium">{{ $t('productDetail.shipping') }}:</span> {{ $t('productDetail.free') }} <router-link to="#" class="text-brand hover:underline">{{ $t('productDetail.moreDetails') }}</router-link>
              </div>
              <div>
                <span class="font-medium">{{ $t('productDetail.location') }}:</span> {{ $t('productDetail.locationValue') }}
              </div>
              <div>
                <span class="font-medium">{{ $t('productDetail.delivery') }}:</span> Zwischen Donnerstag, 11. Dezember und Montag, 15. Dezember, wenn Zahlung heute erhalten wird.
              </div>
              <div>
                <span class="font-medium">{{ $t('productDetail.returns') }}:</span> {{ $t('productDetail.returnsNote') }} <router-link to="#" class="text-brand hover:underline">{{ $t('productDetail.revocationInfo') }}</router-link>
              </div>
            </div>
          </div>
          ...
          <div class="rounded-lg border border-slate-200 bg-white p-4">
            <p class="text-sm font-medium text-slate-900 mb-3">{{ $t('productDetail.paymentMethods') }}:</p>
      ...
      <section v-if="relatedProducts.length > 0" class="mb-8 bg-white rounded-lg border border-slate-200 p-6">
        <h2 class="text-xl font-bold text-slate-900 mb-4">{{ $t('productDetail.similarArticles') }}</h2>
        ...
        <div class="grid md:grid-cols-2 gap-4">
          <div class="space-y-2">
            ...
            <div v-if="product.brand" class="flex justify-between py-2 border-b border-slate-100">
              <span class="text-sm font-medium text-slate-600">{{ $t('productDetail.brand') }}:</span>
      ...
      <section v-if="relatedProducts.length > 6" class="mb-8 bg-white rounded-lg border border-slate-200 p-6">
        <h2 class="text-xl font-bold text-slate-900 mb-4">{{ $t('productDetail.alsoInteresting') }}</h2>
        <div class="grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          <ProductCard
...
import { getProductDetail } from '@theme-classic/data/productDetails'
import { categorySlugs } from '@theme-classic/data/products'
import { useCartStore } from '@theme-classic/stores/cartStore'
import { useWishlistStore } from '@theme-classic/stores/wishlistStore'
import ProductCard from '@theme-classic/component/ProductCard.vue'
// import BestOfferModal from '@theme-classic/components/BestOfferModal.vue'
...
// Related and similar products from API (same category, excluding current product)
...
onMounted(() => {
  // Only report "not found" after loading has finished (avoid false negative while request is in flight)

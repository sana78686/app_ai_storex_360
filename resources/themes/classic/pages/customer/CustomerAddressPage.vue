<template>
  <div>
    ...
    <div class="mx-auto max-w-6xl px-4 py-8">
      <div class="mb-6">
        <h1 class="text-2xl font-bold text-slate-900">Adressen verwalten</h1>
        <p class="mt-2 text-sm text-slate-600">Fügen Sie mehrere Liefer- und Rechnungsadressen hinzu</p>
      </div>
        ...
        <div v-if="activeTab === 0" class="space-y-6">
            ...
            <div class="bg-white rounded-lg border border-slate-200 p-4 sm:p-6">
              <h2 class="text-base sm:text-lg font-semibold text-slate-900 mb-2">
                {{ isEditing && editType === 'delivery' ? 'Adresse bearbeiten' : 'Neue Lieferadresse hinzufügen' }}
              </h2>
              <p class="text-xs sm:text-sm text-slate-600 mb-4">
                Sie können mehrere Adressen speichern und bei der Bestellung auswählen
              </p>
              ...
              <form @submit.prevent="handleSubmitDelivery" class="space-y-3 sm:space-y-4">
                ...
                <div class="flex gap-3 pt-2">
                  ...
                  >
                    {{ isEditing && editType === 'delivery' ? 'Adresse aktualisieren' : 'Adresse hinzufügen' }}
                  </button>
                  <button
            ...
            <div class="bg-white rounded-lg border border-slate-200 p-6">
              <h2 class="text-lg font-semibold text-slate-900 mb-4">
                Gespeicherte Lieferadressen ({{ deliveryAddresses.length }})
              </h2>

              <p v-if="deliveryAddresses.length > 0" class="text-sm text-slate-600 mb-4">
                Klicken Sie auf eine Adresse, um sie auszuwählen. Die ausgewählte wird bei der Bestellung verwendet.
              </p>
              ...
              <div v-if="deliveryAddresses.length === 0" class="text-center py-12 border-2 border-dashed border-slate-200 rounded-lg">
                ...
                <p class="text-sm text-slate-500">Fügen Sie oben Ihre erste Lieferadresse hinzu</p>
              </div>
              ...
              >
                <div class="flex items-center gap-2">
                  ...
                  <p class="text-sm font-medium text-green-900">
                    Diese Adresse wird bei Ihrer nächsten Bestellung verwendet
                  </p>
                </div>
              </div>
            </div>
...
const breadcrumbItems = [
  ...
  { label: 'Adressen', to: null }
]

const tabs = [
  { label: 'Lieferadressen', content: '' },
  { label: 'Rechnungsadressen', content: '' }
]
...
const toggleUseDeliveryAddress = (event) => {
  ...
  if (event.target.checked) {
    notificationStore.info('Lieferadresse wird als Rechnungsadresse verwendet', 'Einstellung gespeichert')
  } else {
...
const getSelectedDeliveryAddressText = () => {
  if (!selectedDeliveryId.value) return 'Keine Lieferadresse ausgewählt'
  const address = deliveryAddresses.value.find(a => a.id === selectedDeliveryId.value)
  if (!address) return 'Adresse nicht gefunden'
...
const selectDeliveryAddress = (id) => {
  ...
  if (useDeliveryAddressForBilling.value) {
    notificationStore.info('Lieferadresse ausgewählt und als Rechnungsadresse verwendet', 'Ausgewählt')
  } else {
    notificationStore.info('Lieferadresse ausgewählt', 'Ausgewählt')
  }
}

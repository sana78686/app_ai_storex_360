    ...
    <div class="mx-auto max-w-7xl px-4 py-6">
      ...
      <div class="grid gap-6 lg:grid-cols-2 mb-8">
        ...
        <div class="space-y-6">
          ...
          <!-- Item Condition -->
          <div class="flex items-center gap-2">
            <span class="font-medium text-slate-900">{{ $t('productDetail.condition') }}:</span>
            <span class="text-slate-700">{{ $t('productDetail.new') }}</span>
...
async function syncCartToApi() {
  ...
  try {
    await axiosTenant.post(
      '/cart/items',
      { product_id: product.value.id, quantity: quantity.value },
      { headers: { 'X-Guest-Token': token } }
    )
  } catch (_) {
    // Offline or API error; cart still in Pinia
  }
}
...
const handleAddToCart = async () => {
  ...
  const item = {
    ...
    articleNumber: product.value.articleNumber ?? `ART-${product.value.id}`,
    price: currentPrice.value,
    oldPrice: product.value.oldPrice ?? null,

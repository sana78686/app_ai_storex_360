  ...
  <div>
    ...
    <div class="mx-auto max-w-4xl px-4 py-8">
      <div class="mb-6 flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-bold text-slate-900">{{ $t('customerNotificationsPage.title') }}</h1>
          <p class="mt-2 text-sm text-slate-600">{{ $t('customerNotificationsPage.subtitle') }}</p>
        </div>
        <button
        ...
        >
          {{ $t('customerNotificationsPage.markAllRead') }}
        </button>
      </div>
      ...
      <div class="space-y-4">
        <div v-if="notificationStore.notifications.length === 0" class="text-center py-12 bg-white rounded-lg border border-slate-200">
          ...
          <p class="text-slate-600">{{ $t('customerNotificationsPage.noNotifications') }}</p>
        </div>
...
const breadcrumbItems = computed(() => [
  ...
  { label: t('customerNotificationsPage.breadcrumbNotifications'), to: null }
])
...
const formatDate = (dateString) => {
  ...
  if (minutes < 1) return t('customerNotificationsPage.justNow')
  if (minutes < 60) return t('customerNotificationsPage.minutesAgo', { n: minutes })
  if (hours < 24) return t('customerNotificationsPage.hoursAgo', { n: hours })
  if (days < 7) return t('customerNotificationsPage.daysAgo', { n: days })

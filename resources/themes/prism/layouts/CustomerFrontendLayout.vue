      ...
      <div class="sticky top-0 flex h-screen flex-col">
          ...
          <div class="flex items-center gap-3">
            ...
            <div class="flex-1">
              <div class="flex items-center justify-between gap-3">
                <div>
                  <h1 class="font-bold text-slate-900">{{ $t('customerAccountLayout.myAccount') }}</h1>
                ...
                <router-link
                  ...
                  :aria-label="$t('customerAccountLayout.ariaBackToHome')"
                >
                  <ArrowLeft class="h-4 w-4" />
                  {{ $t('customerAccountLayout.backToHome') }}
                </router-link>
              </div>
            </div>
          </div>
        ...
        <div class="border-t border-slate-200 p-4 space-y-2">
          ...
          >
            ...
            {{ $t('customerAccountLayout.backToShop') }}
          </router-link>
          <button
            @click="handleLogout"
          ...
          >
            ...
            {{ $t('customerAccountLayout.logout') }}
          </button>
        </div>
      </div>
    </aside>
    ...
    <div class="lg:hidden fixed top-0 left-0 right-0 z-50 border-b border-slate-200 bg-white">
      <div class="flex items-center justify-between p-4">
        <div class="flex items-center gap-2">
          <User class="h-6 w-6 text-brand" />
          <span class="font-bold text-slate-900">{{ $t('customerAccountLayout.myAccount') }}</span>
        </div>
        <button
...
const handleLogout = () => {
  authStore.logout()
  notificationStore.success(t('customerAccountLayout.logoutSuccess'), t('customerAccountLayout.logoutTitle'))
  router.push('/')
}

const navigationItems = computed(() => [
  {
    name: t('customerAccountLayout.navOverview'),
    to: '/konto',
    icon: LayoutDashboard
  },
  {
    name: t('customerAccountLayout.navOrdersReturns'),
    to: '/konto/bestellungen',
    icon: Package
  },
  {
    name: t('customerAccountLayout.navWishlist'),
    to: '/konto/wunschliste',
    icon: Heart
  },
  {
    name: t('customerAccountLayout.navPaymentMethods'),
    to: '/konto/zahlungen',
    icon: CreditCard
  },
  {
    name: t('customerAccountLayout.navAddresses'),
    to: '/konto/adressen',
    icon: MapPin
  },
  {
  ...
  {
    name: t('customerAccountLayout.navSettings'),
